...
public class VentaDao {
    ...
    public int RegistrarVenta(Venta v){
        ...
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setString(1, v.getClienteVenta());
            ps.setString(2, v.getVendedorVenta());
...
public int RegistrarDetalle(Detalle Dv){
       String sql = "INSERT INTO detalle (CodProd, Cantidad, Precio, IdVenta) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setString(1, Dv.getCod_proDetalle());
            ps.setInt(2, Dv.getCantidadDetalle());
            ps.setDouble(3, Dv.getPrecioDetalle());
    ...
    public List Listarventas(){
       ...
       try {
           ...
           while (rs.next()) {               
               ...
               vent.setClienteVenta(rs.getString("Cliente"));
               vent.setVendedorVenta(rs.getString("Vendedor"));
               vent.setTotalVenta(rs.getDouble("Total"));
               ListaVenta.add(vent);
           }
       } catch (SQLException e) {
           System.out.println(e.toString());
